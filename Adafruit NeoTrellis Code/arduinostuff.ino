...
TrellisCallback wave(keyEvent evt)
{
    if(evt.bit.EDGE == SEESAW_KEYPAD_EDGE_RISING)
    {
        ...
        for (int st = 0; st < 15; st++)
        {
            ...
            {
                int key = stepStart[st] + ((light) * 7);
                trellis.setPixelColor(key, Wheel(map(stepStart[st] + ((light) * 7), 0, X_DIM*Y_DIM, 0, 255)));
                trellis.activateKey(key, SEESAW_KEYPAD_EDGE_RISING, false);
                trellis.activateKey(key, SEESAW_KEYPAD_EDGE_FALLING, false);
            }
            trellis.show();
            delay(50);
        }
        ...
        for (int st = 0; st < 15; st++)
        {
            ...
            for (int light = 0; light < RowLen[st] ; light++)
            {
                ...
                trellis.activateKey(key, SEESAW_KEYPAD_EDGE_RISING, true);
                trellis.activateKey(key, SEESAW_KEYPAD_EDGE_FALLING, true);
            }
...
void disableKeys()
{
    ...
    {
        trellis.activateKey(x, SEESAW_KEYPAD_EDGE_RISING, false);
        trellis.activateKey(x, SEESAW_KEYPAD_EDGE_FALLING, false);
    }
}
...
void enableKeys()
{
    for(int x = 0; x<X_DIM * Y_DIM; x++)
    {
        trellis.activateKey(x, SEESAW_KEYPAD_EDGE_RISING, true);
        trellis.activateKey(x, SEESAW_KEYPAD_EDGE_FALLING, true);
    }
}
